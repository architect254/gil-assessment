<div class="space-y-3">
    <div>
        <x-filament::input.wrapper>
            <input
                type="search"
                placeholder="Search name, ID / passport or phone…"
                class="fi-input"
            ...
            <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04] dark:shadow-black/30">
                <header class="flex items-start justify-between gap-3">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <h3 class="truncate text-base font-semibold text-gray-950 dark:text-white">{{ $record->name }}</h3>
                            <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ $record->phone ?? '—' }}</p>
                        </div>
                    </div>
                </header>
                ...
                <dl class="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
                    <div class="flex items-baseline justify-between gap-3">
                        <dt class="text-gray-500 dark:text-gray-400">ID / Passport No.</dt>
                        <dd class="text-right text-gray-950 dark:text-white">{{ $record->id_number ?? '—' }}</dd>
                    </div>
                    <div class="flex items-baseline justify-between gap-3">
                        <dt class="text-gray-500 dark:text-gray-400">Assigned Vehicles</dt>
                        <dd class="text-right font-semibold text-gray-950 dark:text-white">{{ $record->vehicles_count }}</dd>
                    </div>
                </dl>
                ...
                <footer class="mt-4 flex flex-col gap-2 border-t border-gray-100 pt-3 dark:border-white/10">
                    <x-filament::button
                        tag="a"
                        :href="route('filament.gate.resources.drivers.edit', ['record' => $record])"
                        color="gray"
                    >
                        Edit
                    </x-filament::button>
